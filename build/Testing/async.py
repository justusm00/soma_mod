...
def run(test,extra_flags):
    ret = sp.call(["/opt/homebrew/bin/mpiexec","-n", "1",SOMA,"-c","async.h5","-t","0"])
    if ret != 0: raise RuntimeError("Test failed")
    ret = sp.call(["mv","end.h5","async.h5"])
    if ret != 0: raise RuntimeError("Test failed")

    call_list = ["/opt/homebrew/bin/mpiexec","-n","2",SOMA,"-c","async.h5","-t","10000","-a","async_ana_normal.h5","-r","1","--set-generation-algorithm=FIXED-N-SETS","--iteration-alg=SET"]+extra_flags
    print(call_list)
    ret = sp.call(call_list)
    if ret != 0: raise RuntimeError("Test failed")
    ret = sp.call(["mv","end.h5","async_normal.h5"])
    if ret != 0: raise RuntimeError("Test failed")
    call_list = ["/opt/homebrew/bin/mpiexec","-n","2",SOMA,"-c","async.h5","-t","10000","-a","async_ana_new.h5","-r","1","--set-generation-algorithm=FIXED-N-SETS","--iteration-alg=SET",test]+extra_flags
    print(call_list)
    ret = sp.call(call_list)
    if ret != 0: raise RuntimeError("Test failed")
