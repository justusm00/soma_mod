...
void count_monomer_type_fraction(struct Phase *const p, soma_scalar_t * const monomer_type_fraction)
{
    ...
    uint64_t *const monomer_type_count =
        (uint64_t * const)malloc(p->n_types * (p->ana_info.mtf_tested_type_N + 1) * sizeof(uint64_t));
...
#pragma acc exit data copyout(monomer_type_count[0:p->n_types * (p->ana_info.mtf_tested_type_N + 1)])
    ...
    if (p->info_MPI.sim_rank == 0)
        status =
            MPI_Reduce(MPI_IN_PLACE, monomer_type_fraction, p->n_types * (p->ana_info.mtf_tested_type_N + 1),
                       MPI_SOMA_SCALAR, MPI_SUM, 0, p->info_MPI.SOMA_comm_sim);
    else
        status =
            MPI_Reduce(monomer_type_fraction, NULL, p->n_types * (p->ana_info.mtf_tested_type_N + 1), MPI_SOMA_SCALAR,
                       MPI_SUM, 0, p->info_MPI.SOMA_comm_sim);
...
int analytics(struct Phase *const p)
{
    ...
    if (p->ana_info.delta_mc_num_conversions != 0 && p->time % p->ana_info.delta_mc_num_conversions == 0)
        {
            update_self_phase(p, 0);
            unsigned int * const num_conversions = (unsigned int * const)malloc(p->n_poly_type * p->n_poly_type* sizeof(unsigned int)); 
            if (num_conversions == NULL)
                {
                    fprintf(stderr, "ERROR: Malloc %s:%d \n", __FILE__, __LINE__);
                    return -2;
                }           
            unsigned int * const tot_conversions = (unsigned int * const)malloc(p->n_poly_type * p->n_poly_type * sizeof(unsigned int));
            if (tot_conversions == NULL)
                {
                    fprintf(stderr, "ERROR: Malloc %s:%d \n", __FILE__, __LINE__);
                    return -2;
                }
            ...
            for(int i = 0; i<p->n_poly_type*p->n_poly_type;i++)
                {
                    num_conversions[i]=p->pc.num_conversions[i];
                }

#if ( ENABLE_MPI == 1 )
            MPI_Reduce(num_conversions,tot_conversions,p->n_poly_type * p->n_poly_type,MPI_UNSIGNED,MPI_SUM,0,p->info_MPI.SOMA_comm_world);
#endif
            //need soma_scalar_t
            
            soma_scalar_t *tot_conversions_soma=(soma_scalar_t * const)malloc(p->n_poly_type * p->n_poly_type * sizeof(soma_scalar_t)); 
            for(int i=0;i <p->n_poly_type* p->n_poly_type;i++)
                {
                    tot_conversions_soma[i]=(soma_scalar_t)tot_conversions[i];
                }
            if (p->info_MPI.sim_rank == 0)
                extent_ana_by_field(tot_conversions_soma, p->n_poly_type*p->n_poly_type, "/num_conversions", p->ana_info.file_id);

            written = true;
            free(tot_conversions_soma);
            free(tot_conversions);
            free(num_conversions);
        }
...
#pragma acc update self(p->fields_unified[0:p->n_cells*p->n_types])
    ...
    // Monomer type count
    if (p->ana_info.delta_mc_mono_type_fraction != 0 && p->time % p->ana_info.delta_mc_mono_type_fraction == 0)
        {
            //update_self_phase(p,0); //update not needed, because the calculation is on the device
            soma_scalar_t *const monomer_type_fraction =
                (soma_scalar_t * const)malloc(p->n_types * (p->ana_info.mtf_tested_type_N + 1) * sizeof(soma_scalar_t));
            if (monomer_type_fraction == NULL)
                {
                    fprintf(stderr, "ERROR: Malloc %s:%d \n", __FILE__, __LINE__);
                    return -2;
                }
            count_monomer_type_fraction(p, monomer_type_fraction);
            if (p->info_MPI.sim_rank == 0)
                {
                    extent_ana_by_field(monomer_type_fraction, p->n_types * (p->ana_info.mtf_tested_type_N + 1),
                                        "/monomer_type_fraction", p->ana_info.file_id);
                }
            written = true;
