...
int init_ana(struct Phase *const p, const char *const filename, const char *const coord_filename)
{
    ...
    p->ana_info.delta_mc_num_conversions = 0;
...
#define SOMA_NUM_OBS 15
    ...
    names[14] = "/num_conversions";
    delta_mc[14] = &(p->ana_info.delta_mc_num_conversions);
    //******** END EDIT FOR NEW OBSERVABLES HERE************
...
#pragma acc update device(p->ana_info)
    ...
    if (p->ana_info.delta_mc_num_conversions > 0)
        {
            const unsigned int tmp = p->ana_info.delta_mc_num_conversions;
            p->ana_info.delta_mc_num_conversions = 0;
            hid_t dataset = H5Dopen2(p->ana_info.file_id, "/num_conversions", H5P_DEFAULT);
            hid_t status = dataset;
            HDF5_ERROR_CHECK(status);
            hid_t d_space = H5Dget_space(dataset);
            const unsigned int ndims = H5Sget_simple_extent_ndims(d_space);
            if (ndims != 2)
                {
                    fprintf(stderr, "ERROR: %s:%d wrong dimensions for num_conversion.\n", __FILE__, __LINE__);
                    return -1;
                }      
            hsize_t dims[2];    //ndims
            status = H5Sget_simple_extent_dims(d_space, dims, NULL);
            ...
            p->ana_info.delta_mc_num_conversions = tmp;      
        }
    ...
    if (p->ana_info.delta_mc_bonded_energy > 0)
        {                       //Sanitize input
            ...
            if (dims[1] != NUMBER_SOMA_BOND_TYPES)
                {
                    fprintf(stderr,
                            "Error: bonded energy dataspace is incorrect. (Might be an incompatible version problem.)\n");
                    return -2;
                }
            status = H5Dclose(dataset);
            HDF5_ERROR_CHECK(status);
    ...
    if (p->ana_info.delta_mc_dynamical_structure > 0)
        {
            ...
            if (q_size_exists > 0)
                {               //q_size attr exists
                    hid_t q_size_attr =
                        H5Aopen_by_name(dataset, "/dynamical_structure_factor", "q_size", H5P_DEFAULT, H5P_DEFAULT);
            ...
            if (q_exists > 0)
                {               //q attr exists
                    hid_t q_attr =
                        H5Aopen_by_name(dataset, "/dynamical_structure_factor", "q", H5P_DEFAULT, H5P_DEFAULT);
    ...
    if (p->ana_info.delta_mc_static_structure > 0)
        {
            ...
            if (q_size_exists > 0)
                {               //q_size attr exists
                    hid_t q_size_attr =
                        H5Aopen_by_name(dataset, "/static_structure_factor", "q_size", H5P_DEFAULT, H5P_DEFAULT);
                    hid_t d_space = H5Aget_space(q_size_attr);
                    HDF5_ERROR_CHECK(d_space);
                    hsize_t dims[1];    //ndims
                    status = H5Sget_simple_extent_dims(d_space, dims, NULL);
